function el_modulation = ElectModulation(sbj_name,project_name, block_names, dirs, tag, column, conds, preds, datatype, freq_band, mod_par)
...
for ii = 1%:length(mod_par.preds.(conds{i}))
    for imod = 1:length(mod_par.dep_vars.(mod_par.conds{ii}))
