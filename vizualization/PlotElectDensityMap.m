function PlotElectDensityMap(elinfo_all,atlas, plot_elects)
    ...
    for i = ip
        ...
        if plot_elects
            ...
            for ii = 1:size(elinfo.(hemis{hi}),1)
                plot3(elinfo.(hemis{hi}).MNI_coord(ii,1),elinfo.(hemis{hi}).MNI_coord(ii,2),elinfo.(hemis{hi}).MNI_coord(ii,3), 'o', 'MarkerSize', marker_size, 'MarkerFaceColor', colsh.(hemis{hi})(str2num(elinfo.(hemis{hi}).(atlas){ii}),:), 'MarkerEdgeColor', colsh.(hemis{hi})(str2num(elinfo.(hemis{hi}).(atlas){ii}),:), 'LineWidth', 0.1);
            end
        else
        end
    end
...
if plot_elects
    savePNG(gcf,400, ['/Users/pinheirochagas/Desktop/' 'density_map_' atlas 'plot_elects.png'])
else
    savePNG(gcf,400, ['/Users/pinheirochagas/Desktop/' 'density_map_' atlas '.png'])
end
...
savePNG(gcf,400, ['/Users/pinheirochagas/Desktop/' 'density_map_' atlas 'colorbar.png'])
