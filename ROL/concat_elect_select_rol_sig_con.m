...
for is = 1:length(subjects)
    ...
    if exist(fname) == 2
        ...
        % add sig concentration
        fname = sprintf('%ssignal_concentration/%s_%s_signal_concentration.mat',dirs.result_dir, s, task);  
        load(fname)
        cond_names = fieldnames(sig_con);
        for ir1 = 1:length(cond_names)
            cond = cond_names{ir1};
            sig_con_idx = fieldnames(sig_con.(cond));
            for ir2 = 1:length(sig_con_idx)
                sig_con_tmp_name = sprintf('sig_con_%s_%s', cond, sig_con_idx{ir2});
                el_selectivity_tmp.(sig_con_tmp_name) = sig_con.(cond).(sig_con_idx{ir2})';
            end
        end        
