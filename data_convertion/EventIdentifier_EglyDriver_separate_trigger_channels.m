function EventIdentifier_EglyDriver_separate_trigger_channels (sbj_name, project_name, block_names, dirs, pdio_chan)
...
for i = 1:length(block_names)
    ...
    pdio_5 = load(sprintf('%s/Pdio%s_%.2d.mat',globalVar.originalData, bn, pdio_chan)); % going to be present in the globalVar
    pdio_1 = load(sprintf('%s/Pdio%s_%.2d.mat',globalVar.originalData, bn, 1)); % going to be present in the globalVar

    pdio = pdio_5.anlg + pdio_1.anlg;
    %% varout is anlg (single precision)
%     pdio = anlg/max(double(anlg));
    pdio = pdio/max(double(pdio));
...
%     load(['/Volumes/LBCN8T/Stanford/data/psychData/S20_148_SM/E20-225_0014' '/trialinfo_', 'E20-225_0014' '.mat'], 'trialinfo');

%     trialinfo = trialinfo(6:end,:);
