...
deleteContinuousData(sbj_name, dirs, project_name, block_names, 'Spec', 'Spec')
...
fsDir_local = '/usr/local/freesurfer/subjects/fsaverage';
cortex = getcort(dirs);
coords = importCoordsFreesurfer(dirs);
V = importVolumes(dirs);

[elect_names, DRMT1] = importElectNames(dirs);

isSubdural = [];
for i = 1:length(DRMT1.VarName2)-2
    if strcmp(DRMT1.VarName2(i+2),'D')
        isSubdural(i,1) = 0;
    else
        isSubdural(i,1) = 1;
    end
...
isLeft = [];
for i = 1:length(DRMT1.DepthStripGrid)-2
    if strcmp(DRMT1.DepthStripGrid(i+2),'L')
        isLeft(i,1) = 1;
    else
        isLeft(i,1) = 0;
    end
...
mniCfg.elecCoord = coords;
mniCfg.elecNames = elect_names;
mniCfg.isLeft = isLeft;
mniCfg.isSubdural = isSubdural;


% Convert electrode coordinates from native to MNI space
[MNI_coords, elecNames, isLeft, avgVids, subVids] = sub2AvgBrainCustom(mniCfg,dirs, fsDir_local);
...
data_all.wave = data_all.wave(~contains(trialinfo.wlist, '�'),:,:); % correct that for MMR
trialinfo = trialinfo(~contains(trialinfo.wlist, '�'),:); % correct that for MMR
